...
def find_similar_users(current_user):
    ...
    current_user_exclude_pks = list(current_user.approved_users.values_list('pk', flat=True)) + list(current_user.denied_users.values_list('pk', flat=True)) + [current_user.pk]
